...
def validate_section(text):
    ...
    while True:
        ...
        if re.match(pattern, section):
            ...
            print("Error: The section must have the correct format (e.g. 10B or 5A). Please enter a valid section.")
...
def validate_not_duplicate(students, new_student):   
    ...
    # Ensure existing list contains only Student instances
    for student in students:        

        if (student.name.lower() == new_student.name.lower() and student.section.upper() == new_student.section.upper()):
            print(f"Error: The student {new_student.name} in section {new_student.section} already exists. The student will not be added.")
            return False
